...
def create_invoice():
    ...
    invoice_date = f'{my_date.day}-{my_date.month}-{my_date.year}'
    invoice_text.insert(END, f'Information: \t{invoice_number}\t\t{invoice_date}\n')
    invoice_text.insert(END,f'*'* 65 +'\n')
...
#Window Size
application.geometry("1920x1080+0+0")
...
#Left Panel
left_panel = Frame(application,bd=1,relief=FLAT,padx=50)
...
#Product lists
food_list = ['Chicken','Lamb','Salmon','Make','Kebab','Pizza1','Pizza2','Pizza3']
drink_list = ['Lemonade','Soda','Juice','Cola','Wine1','Wine2','Beer1','Beer2']
dessert_list = ['Ice Cream','Fruits','Brownies','Pudding','Cheesecake','Cake1','Cake2','Cake3']
...
for food in food_list:
    ...
    food_box[counter] = Entry(food_panel,font=('Dosis',18,'bold'),bd=1,width=6,state=DISABLED,textvariable=food_variables[counter])
...
for drink in drink_list:
    ...
    drink_box[counter] = Entry(drink_panel, font=('Dosis', 18, 'bold'), bd=1, width=6, state=DISABLED,
                              textvariable=drink_variables[counter])
...
for dessert in dessert_list:
    ...
    dessert_box[counter] = Entry(dessert_panel, font=('Dosis', 18, 'bold'), bd=1, width=6, state=DISABLED,
                               textvariable=dessert_variables[counter])
...
food_cost_text = Entry(cost_panel,font=('Dosis',12,'bold'),bd=1,width=27,state='readonly',textvariable=food_cost_var)
...
drink_cost_text = Entry(cost_panel,font=('Dosis',12,'bold'),bd=1,width=27,state='readonly',textvariable=drink_cost_var)
...
dessert_cost_text = Entry(cost_panel,font=('Dosis',12,'bold'),bd=1,width=27,state='readonly',textvariable=dessert_cost_var)
dessert_cost_text.grid(row=2, column=1, padx=41)

#Subtotal
subtotal_cost_label = Label(cost_panel,text="Subtotal",font=('Dosis',12,'bold'),bg='azure4',fg='white')
subtotal_cost_label.grid(row=0, column=2)
subtotal_cost_text = Entry(cost_panel,font=('Dosis',12,'bold'),bd=1,width=27,state='readonly',textvariable=subtotal_var)
...
subtotal_cost_label.grid(row=1, column=2)
subtotal_cost_text = Entry(cost_panel,font=('Dosis',12,'bold'),bd=1,width=27,state='readonly',textvariable=subtotal_var)
subtotal_cost_text.grid(row=1, column=3, padx=41)
...
total_cost_text = Entry(cost_panel,font=('Dosis',12,'bold'),bd=1,width=27,state='readonly',textvariable=total_var)
...
for button in buttons:
    button = Button(button_panel, text=button.title(),font=('Dosis',14,'bold'),fg='grey',bg='azure4',bd=1,width=9)
...
#Invoice area
invoice_text = Text(invoice_panel,font=('Dosis',12,'bold'),width=42,height=10)
invoice_text.grid(row=0, column=0)

#Calculator
calculator_display = Entry(calculator_panel,font=('Dosis',12,'bold'),bd=1,width=32)
...
for button in calculator_buttons:
    button = Button(calculator_panel,text=button.title(),font='Dosis',fg='grey',bg='azure4',bd=1,width=8)
